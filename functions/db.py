...
class Table(metaclass=TableMeta):
  ...
  async def create_pool(cls, *, prod=False, canary=False, **kwargs):
    ...
    kwargs.update({
        "host": 'localhost' if prod or canary else os.environ["DBHOSTNAME"],
        "user": os.environ["DBUSERNAMECANARY"] if canary else os.environ["DBUSERNAME"] if prod else os.environ["DBUSERNAMELOCAL"],
        "password": os.environ["DBPASSWORDCANARY"] if canary else os.environ["DBPASSWORD"] if prod else os.environ["DBPASSWORDLOCAL"],
        "database": os.environ["DBDATABASECANARY"] if canary else os.environ["DBDATABASE"] if prod else os.environ["DBDATABASELOCAL"],
        'command_timeout': 60,
        'max_size': 20,
        'min_size': 20,
    })
    ...
    cls._pool = pool = await asyncpg.create_pool(init=init, **kwargs)
    return pool
