...
class RoboPages(discord.ui.View):
  ...
  async def show_page(self, button: discord.Button, page_number: int) -> None:
    page = await self.source.get_page(page_number)
    self.current_page = page_number
    kwargs = await self._get_kwargs_from_page(page)
    self._update_labels(page_number)
    if kwargs:
      if button.response.is_done():
        if self.message:
          await self.message.edit(**kwargs, view=self)
      else:
        await button.response.edit_message(**kwargs, view=self)
  ...
  #     await button.response.send_message('Already waiting for your response...', ephemeral=True)
  #     return
  ...
  #     author_id = button.user and button.user.id
  #     await button.response.send_message('What page do you want to go to?', ephemeral=True)
  ...
  #       await button.followup.send('Took too long.', ephemeral=True)
