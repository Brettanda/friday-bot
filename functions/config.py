...
def welcome_message(bot: "Bot", *, prefix: str = defaultPrefix) -> dict:
  ...
  return dict(embed=embed(
      ...
      description=f"I will respond to messages when I am mentioned. To get started with commands type `{prefix}help` or `@{bot.user.name} help`.\n"
      f"If something goes terribly wrong and you want it to stop, talk to my creator with the command `{prefix}issue <message>`",
      thumbnail=bot.user.avatar.url if hasattr(bot.user, "avatar") else bot.user.avatar_url if hasattr(bot.user, "avatar_url") else None,
      fieldstitle=["Prefix", "Mute me", "Setting a language", "Notice for chat system"],
      fieldsval=[
          f"To change my prefix use the `{prefix}prefix` command.",
          f"If I start bothering people with messages use the `{prefix}set mute` command.",
