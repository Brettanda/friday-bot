...
class ServerSettings(commands.Cog):
  ...
  async def server(self,ctx):
    await ctx.reply(embed=embed(title="Invalid subcommand",color=MessageColors.ERROR),mention_author=False)
  ...
  async def defaultrole(self,ctx,role:discord.Role):
    await ctx.reply(embed=embed(title=f"The new default role for new members is `{role}`"),mention_author=False)
