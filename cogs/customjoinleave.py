...
class CustomJoinLeave(commands.Cog):
  ...
  async def custom_join(self, ctx, url: typing.Optional[str] = None):
    async with ctx.typing():
      ...
      reactions = await self.bot.db.query("SELECT customJoinLeave FROM servers WHERE id=$1", ctx.guild.id)
      ...
      await self.bot.db.query("UPDATE servers SET customJoinLeave=$1 WHERE id=$2", json.dumps(reactions), ctx.guild.id)
  ...
  #   reactions = await query(self.bot.log.mydb,"SELECT customJoinLeave FROM servers WHERE id=$1",member.guild.id)
  #   reactions = json.loads(reactions)
