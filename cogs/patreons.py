...
from functions.config import ChatSpamConfig, PremiumTiersNew
...
class Patreons(commands.Cog):
  ...
  async def norm_patreon(self, ctx: GuildContext):
    ...
    await ctx.send(embed=embed(
        ...
        fieldsval=[f"Get up to {ChatSpamConfig.patron_3_rate} messages/12 hours instead of {ChatSpamConfig.free_rate} messages/12 hours.", "Change the persona that Friday uses when chatting in your server", "You will be granted role(s) in the support server.", "Get priority support for your encounters with Friday", "Get the full list of benefits on Patreon."],
        fieldsin=[False] * 5,
